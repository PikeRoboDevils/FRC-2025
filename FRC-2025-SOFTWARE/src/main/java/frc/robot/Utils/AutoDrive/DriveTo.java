...
public class DriveTo {
  ...
  public Command getPathFromWaypoint(Pose2d waypoint) {
    List<Waypoint> waypoints =
        PathPlannerPath.waypointsFromPoses(
            new Pose2d(
                mSwerve.getVisionPose().getTranslation(),
                getPathVelocityHeading(mSwerve.getFieldVelocity(), waypoint)),
            waypoint);
    ...
    PathPlannerPath path =
        new PathPlannerPath(
            ...
            new IdealStartingState(
                getVelocityMagnitude(mSwerve.getFieldVelocity()), mSwerve.getHeading()),
  ...
  /** Will go straight to the given pose not much logic behind it */
