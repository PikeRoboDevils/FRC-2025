...
public class AlignToReef extends DriveTo {
  ...
  public AlignToReef(Swerve mSwerve, AprilTagFieldLayout field) {
    ...
    Arrays.stream(new int[] {6, 7, 8, 9, 10, 11})
        .forEach(
            (i) -> {
              field
                  .getTagPose(i)
    ...
    Arrays.stream(new int[] {17, 18, 19, 20, 21, 22})
        .forEach(
            (i) -> {
              field
                  .getTagPose(i)
    ...
    Arrays.stream((new int[] {6, 7, 8, 9, 10, 11, 17, 18, 19, 20, 21, 22}))
        .forEach(
            (i) -> {
              field
                  .getTagPose(i)
    ...
    /*
     * In the future we should use a util file just for positions of april tags
  ...
  public static Pose2d getClosestReefAprilTag(Pose2d pose) {
    ...
    if (alliance.isEmpty()) {
      ...
      reefPoseList = alliance.get() == Alliance.Red ? blueReefTagPoses : redReefTagPoses;
    }
