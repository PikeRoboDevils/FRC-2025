...
public class WristSim implements WristIO {
    ...
    public WristSim() {
        _wrist = new SingleJointedArmSim(DCMotor.getNEO(1),
         50,
        SingleJointedArmSim.estimateMOI(Units.inchesToMeters(8),
         5),
    ...
    public void updateInputs(WristIOInputs inputs) {
        ...
        inputs.WristVolt = _wrist.getInput(0); //I think this is the reference for voltage. Still needs checked, it would apear to be
        inputs.WristVelocity = Units.radiansPerSecondToRotationsPerMinute(_wrist.getVelocityRadPerSec());
    }

    @Override
    public void setAngle(double angleDeg) {
        double ffw = _feedforward.calculate(Units.degreesToRadians(angleDeg), _wrist.getAngleRads() - Units.degreesToRadians(angleDeg));
